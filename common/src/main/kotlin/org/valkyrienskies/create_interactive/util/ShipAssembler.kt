package org.valkyrienskies.create_interactive.util
...
object ShipAssembler {
    ...
    private fun bfs(
        ...
        while (!stack.isEmpty) {
            ...
            if ((CreateInteractiveConfig.SERVER.maxShipBlocks > 0) and (blocks.size > CreateInteractiveConfig.SERVER.maxShipBlocks)) {
                logger.info("Stopped ship assembly due too many blocks")
                return false
            }
        }
        if (CreateInteractiveConfig.SERVER.maxShipBlocks > 0) {
            logger.info("Assembled ship with ${blocks.size} blocks, out of ${CreateInteractiveConfig.SERVER.maxShipBlocks} allowed")
        }
        return true
    }

    private fun directions(center: BlockPos, lambda: (BlockPos) -> Unit) {
        if (!CreateInteractiveConfig.SERVER.diagonals) Direction.values().forEach { lambda(center.relative(it)) }
        for (x in -1..1) {
            for (y in -1..1) {
                for (z in -1..1) {
                    if (x != 0 || y != 0 || z != 0) {
