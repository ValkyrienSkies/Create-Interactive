...
class EurekaShipControl : ShipForcesInducer, ServerTickListener {
    ...
    override fun applyForces(physShip: PhysShip) {
        ...
        physShip.doFluidDrag = CreateInteractiveConfig.SERVER.doFluidDrag
        ...
        val buoyantFactorPerFloater = min(
            CreateInteractiveConfig.SERVER.floaterBuoyantFactorPerKg / 15 / mass,
            CreateInteractiveConfig.SERVER.maxFloaterBuoyantFactor
        )
        ...
        if ((aligning) && abs(angleUntilAligned) > ALIGN_THRESHOLD) {
            ...
            val idealOmega = Vector3d(invRotationAxisAngle.x, invRotationAxisAngle.y, invRotationAxisAngle.z)
                ...
                .mul(CreateInteractiveConfig.SERVER.stabilizationSpeed)
        ...
        controlData?.let { control ->
            ...
            val largestDistance = run {
                ...
                dist
            }.coerceIn(0.5, CreateInteractiveConfig.SERVER.maxSizeForTurnSpeedPenalty)

            val maxLinearAcceleration = CreateInteractiveConfig.SERVER.turnAcceleration
            val maxLinearSpeed = CreateInteractiveConfig.SERVER.turnSpeed + extraForceAngular
            ...
            val baseForwardVel = Vector3d(forwardVector).mul(CreateInteractiveConfig.SERVER.baseSpeed)
            ...
            val idealForwardVel = Vector3d(forwardVector).mul(CreateInteractiveConfig.SERVER.maxCasualSpeed)
            ...
            if (control.upImpulse != 0.0f) {
                idealUpwardVel = Vector3d(0.0, 1.0, 0.0)
                    ...
                    .mul(
                        CreateInteractiveConfig.SERVER.baseImpulseElevationRate +
                            // Smoothing for how the elevation scales as you approaches the balloonElevationMaxSpeed
                            smoothing(2.0, CreateInteractiveConfig.SERVER.balloonElevationMaxSpeed, balloonForceProvided / mass)
                    )
            }
        }
        ...
        val idealUpwardForce = Vector3d(
            0.0,
            idealUpwardVel.y() - vel.y() - (GRAVITY / CreateInteractiveConfig.SERVER.elevationSnappiness),
            0.0
        ).mul(mass * CreateInteractiveConfig.SERVER.elevationSnappiness)
    ...
    private fun showCruiseStatus() {
        val cruiseKey = if (isCruising) "hud.create_interactive.start_cruising" else "hud.create_interactive.stop_cruising"
    ...
    companion object {
        ...
        private val forcePerBalloon get() = CreateInteractiveConfig.SERVER.massPerBalloon * -GRAVITY
