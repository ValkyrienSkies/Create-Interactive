package org.valkyrienskies.create_interactive.blockentity
...
import org.valkyrienskies.create_interactive.EurekaProperties.HEAT
import org.valkyrienskies.create_interactive.gui.engine.EngineScreenMenu
import org.valkyrienskies.create_interactive.ship.EurekaShipControl
import org.valkyrienskies.create_interactive.util.KtContainerData
import org.valkyrienskies.mod.common.getShipManagingPos
import kotlin.math.ceil
import kotlin.math.max

class EngineBlockEntity(pos: BlockPos, state: BlockState) :
    BaseContainerBlockEntity(CreateInteractiveBlockEntities.ENGINE.get(), pos, state),
    ...
    private var maxEffectiveFuel = 100f - CreateInteractiveConfig.SERVER.engineHeatGain
    private var lastFuelValue = 1600; // coal: 1600

    override fun createMenu(containerId: Int, inventory: Inventory): AbstractContainerMenu =
        EngineScreenMenu(containerId, inventory, this)

    override fun getDefaultName(): Component = TranslatableComponent("gui.create_interactive.engine")
    ...
    fun tick() {
        ...
        if (CreateInteractiveConfig.SERVER.engineRedstoneBehaviorPause && isPowered) return

        // Disable engine feeding when they are receiving a redstone signal
        if (!isPowered) {
            if (fuelLeft > 0) {

                if (CreateInteractiveConfig.SERVER.engineFuelSaving) {
                    if (heat <= maxEffectiveFuel) {
                        heat += scaleEngineHeating(CreateInteractiveConfig.SERVER.engineHeatGain)
                        fuelLeft--
                    }
                } else {
                    fuelLeft--
                ...
                if (!fuel.isEmpty && lastFuelValue <= CreateInteractiveConfig.SERVER.engineMinCapacity - fuelLeft) {
                    consumeFuel()
                }
        ...
        if (heat > 0) {
            ...
            if (ship != null && eurekaShipControl != null) {
                ...
                eurekaShipControl.powerLinear += lerp(
                    CreateInteractiveConfig.SERVER.enginePowerLinearMin,
                    CreateInteractiveConfig.SERVER.enginePowerLinear,
                    effectiveHeat,
                )

                eurekaShipControl.powerAngular += lerp(
                    CreateInteractiveConfig.SERVER.enginePowerAngularMin,
                    CreateInteractiveConfig.SERVER.enginePowerAngular,
                    effectiveHeat,
                )
            ...
            heat = max(heat - scaleEngineCooling(CreateInteractiveConfig.SERVER.engineHeatLoss), 0f)
        }
    }
    ...
    private fun getScaledFuel(): Int =
        ((FurnaceBlockEntity.getFuel()[fuel.item] ?: 0) * CreateInteractiveConfig.SERVER.engineFuelMultiplier).toInt()
    ...
    private fun consumeFuel() {
        ...
        if (lastFuelValue > 0) {
            if (fuelLeft > 0 && lastFuelValue > CreateInteractiveConfig.SERVER.engineMinCapacity - fuelLeft) {
                return
            }

            fuelLeft += lastFuelValue
            fuelTotal = max(lastFuelValue, CreateInteractiveConfig.SERVER.engineMinCapacity)
    ...
    private fun scaleEngineHeating(value: Float): Float =
        (100 * CreateInteractiveConfig.SERVER.engineHeatChangeExponent - this.heat * CreateInteractiveConfig.SERVER.engineHeatChangeExponent + 1f) * value
    ...
    private fun scaleEngineCooling(value: Float): Float =
        (this.heat * CreateInteractiveConfig.SERVER.engineHeatChangeExponent + 1f) * value
